...
import { noop } from './svelte_internals_utils';
import {
  writable,
  get,
  type Readable,
...
export function createFlowStore<S extends Stores>(
  ...
  let current:
    | { reject: (error?: any) => void; unsubscribe?: () => void }
    | undefined = undefined;

  const { subscribe, assign, acknowledgeError } =
    setupWriteableWithError<FlowData>({
      state: 'Idle',
    });

  const execute = async <R>(
    func: ($store: StoresValues<S>) => Promise<R> | R,
  ): Promise<R> => {
...
export function flowFor<S extends Stores>(
  ...
  return (
    func?: (values: StoresValues<S>) => Promise<any> | any,
  ): FlowStore<StoresValues<S>> => {
...
export function setupWriteableWithState<States extends string, T>(data: T) {
  ...
  const st = setupWriteableWithError<
    T & { state: States | 'Idle'; transitioning?: string }
  >(d);
  return {
    ...st,
